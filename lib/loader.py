...
from util.text import exception_to_unicode
...
def load_py_files():
    ...
    def _load_py_files(env, search_path, auto_enable=None):
        for path in search_path:
            ...
            env.log.debug( "_load_py_files found plugins %s" % plugin_files)
            for plugin_file in plugin_files:
                try:
                    ...
                    raise
                    #env.log.debug('Failed to load plugin from %s', plugin_file,
                    #              exception_to_unicode(e, traceback=True))

    return _load_py_files

def load_components(env, extra_path=None, loaders=(load_eggs(''),
                                                   load_py_files())):
    """Load all plugin components found on the given search path."""
    plugins_dir = os.path.normcase(os.path.realpath(
        os.path.join(env.path, 'modules')
    ))
    env.log.debug("Loader got plugin_dir %s" % plugins_dir)
